...
public class TreeViewEditingWithContextMenu extends Application {
	...
	SortedList<Person> data = new SortedList<>(list, (p1, p2) -> {
	...
	private class MyTextFieldTreeCell extends TreeCell<String> {
        ...
        public MyTextFieldTreeCell() {
            MenuItem addMenuItem = new MenuItem("Add Person");
            addMenu.getItems().add(addMenuItem);
            addMenuItem.setOnAction( e -> {
			    TreeItem<String> newEmployee = new TreeItem<>("New Employee");
			    getTreeItem().getChildren().add(newEmployee);
			    Person p = new Person("", "", "", "");
			    p.setFirstName("New Employee");
			    p.setFavoriteColor(colorMenu);
			    list.add(p);
			});
        }
	       ...
	       if(!getTreeItem().isLeaf()) {			//if the tree item is not a leaf - it has children - we cancel editing
	    	   cancelEdit();
	    	   return;
	       } 
